        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 13006:                                 // Gnomish Shrink Ray (ItemID: 10716)
                {
                    ...
                    if (roll < 2)                                         // Whole party will grow (Now just where did Blizz hide this spell...)
                        m_caster->CastSpell(m_caster, 13004, true);
                    else if (roll < 4)                                    // Whole party will shrink
                        m_caster->CastSpell(m_caster, 13010, true);
                    else if (roll < 6)                                    // Whole enemy 'team' will grow (Now just where did Blizz hide this spell...)
                ...
                {
                    // BROKEN - IS NEVER CASTED BY AURA! NEEDS UPDATE ON CORE HANDLING - (SPELL AURA PERIODIC TRIGGER SPELL)
                    uint32 roll = urand(0, 99);

                    int32 DRMod = 600 + urand((m_caster->GetMaxHealth()/10), (m_caster->GetMaxHealth()/2));

                    if (roll < 15 || !unitTarget)
                        m_caster->CastCustomSpell(m_caster, 13279, &DRMod, nullptr, nullptr, true, m_CastItem);

                    else
                        m_caster->CastCustomSpell(unitTarget, 13279, &DRMod, nullptr, nullptr, true, m_CastItem);
